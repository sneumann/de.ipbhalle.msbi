...
public class PreprocessSpectraLive {
	...
	private Vector<Peak> mergePeaks(Vector<WrapperSpectrum> spectra, double mzabs, double mzppm)
	{
		...
		for (int i = 1; i < peakArray.length; i++) {
			...
			if((currentPeak.getMass() >= (peakArray[i].getMass() - (mzabs + PPMTool.getPPM(currentPeak.getMass(), mzppm)))) && (currentPeak.getMass() <= (peakArray[i].getMass() + (mzabs + PPMTool.getPPM(currentPeak.getMass(), mzppm)))))
			{
